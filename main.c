...
{   
    ...
    OpenADC(ADC_FOSC_32 & ADC_RIGHT_JUST & ADC_2_TAD, ADC_CH0 & ADC_INT_OFF & ADC_VREFPLUS_VDD & ADC_VREFMINUS_VSS, ADC_12ANA);
...
void ReadSensors(BYTE* p)	// Fun��o destinada a leitura de todos os sensores
{
	int Seq;
	unsigned int result;
	...
	switch(Seq){
		case 0:
			SetChanADC( ADC_CH1 );	// canal analogico para analise do potenciomentro: right 01
			break;
		case 1:
			SetChanADC( ADC_CH2 );	// canal analogico para analise do potenciomentro: right 02
			break;
		case 2:
			SetChanADC( ADC_CH3 );	// canal analogico para analise do potenciomentro: left 01
			break;
		case 3:
			SetChanADC( ADC_CH4 );	// canal analogico para analise do potenciomentro: left 02
			break;
		case 4:
			SetChanADC( ADC_CH8 );	// canal analogico para analise do medidor de corrente: right 01
			break;
		case 5:
			SetChanADC( ADC_CH9 );	// canal analogico para analise do medidor de corrente: right 02
			break;
		case 6:
			SetChanADC( ADC_CH10 );	// canal analogico para analise do medidor de corrente: left 01
			break;
		case 7:
			SetChanADC( ADC_CH12	);	// canal analogico para analise do medidor de corrente: left 02
			break;
		}
...
void Potenci(BYTE* p) // Fun��o relacionada a leitura dos potenciomentros com convers�o A/D
{
	int Seq;
	unsigned int result;
...
void Amperimet(BYTE* p) // Fun��o relacionada a leitura dos leitores de corrente com convers�o A/D
{
	int Seq=0;
	unsigned int result=0;

	for(Seq = 0; Seq<8; Seq+=2 )
	{
	switch(Seq){
		case 0:
                        SetChanADC( ADC_CH9 );	// canal analogico para analise do medidor de corrente: right 01
			break;
		case 2:
                        SetChanADC( ADC_CH8 );	// canal analogico para analise do medidor de corrente: right 02
			break;
		case 4:
			SetChanADC( ADC_CH10 );	// canal analogico para analise do medidor de corrente: left 01
			break;
	...
	while( BusyADC() ); // Wait for completion
	result = ReadADC(); // Read result
	p[Seq] = (unsigned int)result;
	p[Seq+1] = (unsigned int)result>>8;
	}	// end FOR
}
    ...
    {   
        switch(ReceivedDataBuffer[0])				//Look at the data the host sent, to see what kind of application specific command it sent.
        {
            ...
            case 0x81:  //Get push button state
                ...
                ToSendDataBuffer[0] = 0x85;				//Echo back to the host PC the command we are fulfilling in the first byte.  In this case, the Get Pushbutton State command.
				SetChanADC( ADC_CH11 );
				ConvertADC();       // Start conversion
				while( BusyADC() ); // Wait for completion
				ToSendDataBuffer[2] = ReadADC(); // Read result				
//				if(sw2 == 1)							//pushbutton not pressed, pull up resistor on circuit board is pulling the PORT pin high
				if(ToSendDataBuffer[2]>0)
				{
						...
						case 0x23:
							...
							switch(ReceivedDataBuffer[1]){
								...
								default: // Fun��o n�o reconhecida
									...
									ToSendDataBuffer[5] = 0xFF;
									ToSendDataBuffer[6] = 0xFF;
									ToSendDataBuffer[7] = 0xFF;
									ToSendDataBuffer[8] = 0xFF;
									ToSendDataBuffer[9] = 0xFF;
									ToSendDataBuffer[10] = 0xFF;
									ToSendDataBuffer[11] = 0xFF;
									ToSendDataBuffer[12] = 0xFF;
									ToSendDataBuffer[13] = 0xFF;
									ToSendDataBuffer[14] = 0xFF;
